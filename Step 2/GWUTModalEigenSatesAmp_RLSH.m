%% Step 2 Program :  Plug the Specic k and w to find the amplitude eigen vectors from Plot after Step 1.3 %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
theta_sv = real(acosd(k/ks)); % wave propagation diraction of S-wave 2
theta_sh = real(acosd(kh/ks)); % wave propagation diraction of S-wave 2

eta=sqrt(kp^2-k^2);
beta_v=sqrt(ks^2-k^2);
beta_h=sqrt(ks^2-kh^2);


E1=exp(1i*h*eta);
E_m1=exp(-1i*h*eta);
B1=exp(1i*h*beta_v);
B_m1=exp(-1i*h*beta_v);

eta=sqrt((w/Cp)^2-k^2);
beta_v=sqrt((w/Cs)^2-k^2);
...
C13=-2*k*beta_v*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B1;
C14=2*k*beta_v*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B_m1;
...
C23=-2*k*beta_v*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B_m1;
C24=2*k*beta_v*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B1;
        
C31=-2*k*eta*(T*(besselj(1,e*eta)/eta)-besselj(0,e*eta))*E1;
C32=2*k*eta*(T*(besselj(1,e*eta)/eta)-besselj(0,e*eta))*E_m1;
C33=(2*k^2-ks^2)*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B1;
C34=(2*k^2-ks^2)*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B_m1;
        
C41=-2*k*eta*(T*(besselj(1,e*eta)/eta)-besselj(0,e*eta))*E_m1;
C42=2*k*eta*(T*(besselj(1,e*eta)/eta)-besselj(0,e*eta))*E1;
C43=(2*k^2-ks^2)*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B_m1;
C44=(2*k^2-ks^2)*(T*(besselj(1,e*beta_v)/beta_v)-besselj(0,e*beta_v))*B1;
        
Crl=[C11 C12 C13 C14; C21 C22 C23 C24; C31 C32 C33 C34; C41 C42 C43 C44];
clear C11 C12 C13 C14 C22 C23 C24 C31 C32 C33 C34 C41 C42 C43 C44 
clear E1 E_m1 B1 B_m1
display(Crl)

        Bh1=exp(1i*h*beta_h);
        Bh_m1=exp(-1i*h*beta_h);
        
        C11=beta_h*besselj(0,e*beta_h)*Bh1;
        C12=beta_h*besselj(0,e*beta_h)*Bh_m1;
        C21=beta_h*besselj(0,e*beta_h)*Bh_m1;
        C22=beta_h*besselj(0,e*beta_h)*Bh1;
        clear Bh1 Bh_m1
...
[U, S, V] = svd(Crl); % this uses the Singular Value DEcomposition which I knew from online. 
[Uh, Sh, Vh] = svd(Ch); % this uses the Singular Value DEcomposition which I knew from online. 

%[eigVec, eigVal] = eig(C); % No need as V has a right null space of the C
%matrix 

AB = V(:,end); % Solution vector A (non-trivial solution if exists) Given Amplitude / Polarity
C=Vh(:,end);
clear Crl Ch
%% 
% Display the solution
disp('Solution vector AB:');
disp(AB);
componentNames_RL = {'Apu', 'Apd', 'Asvu', 'Asvd'};
% Display the solution
disp('Solution vector C:');
disp(C);
componentNames_SH = {'Cu', 'Cd'};
...
psi_SVu = @(x, y) Asvu * exp(1i * (k * x + beta_v * y));
psi_SVd = @(x, y) Asvd * exp(1i * (k * x - beta_v * y));

% Displacements u_x and u_y
u_1 = @(x, y) 1i * k * (phi_pu(x, y) + phi_pd(x, y)) + 1i * beta_v * (psi_SVu(x, y) - psi_SVd(x, y));
